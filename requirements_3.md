...
- Не требуй помечать поля с суфиксом _id атрибутом FK если для них нет связанной таблицы.
...
- Когда ты пишешь замечание, ты должен цитировать какое требование было нарушено
- Если поле помечено на диаграмме как FK, но нет таблицы с которым оно может быть связанно, то в моделях не нужно требовать использование класса ForeignKeyField
- Если ты видишь что название таблицы по смыслу совпадает с сервисом, который не представлен в этой работе, то нужно сообщить об этом как об ошибке, так как информация в этой таблице должна храниться в других сервисах.
- Если связи ведут на сущности в других сервисах, не нужно требовать отображать их на диаграмме
- В БД не должно быть таблиц состоящих из одного первичного ключа
