...
- Все поля `NOT NULL`. Не нужно заполнять параметры значениями, которые совпадают с дефолтными, например `null=False`
- Если есть модель с которой может быть реализован внешний ключ, то его реализовать через `ForeignKeyField`, иначе `IntegerField`
