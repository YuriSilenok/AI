...
- На диаграмме отображаются реляционные связи (пример: `||--o{`, `}o--||` и т.д.), кроме тех случаев, когда связи ведут в другие сервисы.

Если таблиц несколько, то список реляционных связей в которых указано, какие поля из каких таблиц связываются
...
- Запрещены таблицы в doc.md и models.py которые не относятся к текущему сервису.
- Разрешены поля которые заканчиваются на _id у которых нет метки FK и/или связи с другими таблицами.
- Разрешено добавлять больше информации в doc.md, чем описано в требованиях к УП.
- Тебе как нейросети разрешено отвечать только списком критических замечаний или написать "Замечаний нет".
- Ты как нейросеть обязан процитировать, какье требование было нарушено, при написании замечания.
- Тебе как нейросети запрежено предлагать улучшения.
- Считать нормальным, что на проверку предоставлено больше, чем описано в требованиях.
- Если в doc.md поле имеет метку FK, но не имеет связи, то в models.py у этого поля тип должен быть IntegerField.
- Запрещено реализовывать бизнес логику в models.py, но разрешено реализовывать ограничения полоей, обеспечивающих целостность данных.
- Код в файле models.py должен соответствовать версии python 3.8
- Считать что таблица UserRole относится к сервису 3. Role Service (Сервис ролей)
